...
final class CmuxChromiumBrowserView: NSView {
    ...
    private func readContextIDIfAvailable() {
        guard let text = try? String(contentsOf: contextFile, encoding: .utf8),
              ...
              contextID != 0,
              contextID != currentContextID else { return }
    ...
    private func ensureDevToolsPageWebSocketURL(
        ...
        var result: Result<URL, Error> = .failure(CmuxChromiumCoreError.devToolsUnavailable)
        URLSession.shared.dataTask(with: listURL) { data, _, error in
            ...
            guard let data,
                  ...
                  let url = URL(string: webSocket) else {
    ...
    private func sendDevToolsCommand(
        ...
        let lock = NSLock()
        var didFinish = false
        var commandResult: Result<[String: Any], Error> = .failure(
            CmuxChromiumCoreError.devToolsError("Timed out waiting for browser script response")
        )
