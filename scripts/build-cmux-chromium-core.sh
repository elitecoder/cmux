...
DEFAULT_DOWNLOAD_URL="https://github.com/manaflow-ai/chromium/releases/download/owl-chromium-38a10eea4c9e/owl-chromium-runtime-macos-arm64-38a10eea4c9e.tar.gz"
DEFAULT_DOWNLOAD_SHA256="d0d07d859bb2986f1515db0e65a00afc722c02de153319835e7b873e57cb53ab"
...
CACHE_ROOT="${CMUX_CHROMIUM_CONTENT_SHELL_CACHE:-$HOME/Library/Caches/cmux/chromium-content-shell/owl-chromium-38a10eea4c9e}"
...
resolve_content_shell_app() {
  ...
  candidates+=(
    "$CACHE_ROOT/owl-chromium-runtime-macos-arm64-38a10eea4c9e/Content Shell.app"
...
download_content_shell_if_needed() {
  ...
  if [[ "$DOWNLOAD_URL" == "$DEFAULT_DOWNLOAD_URL" ]]; then
    archive="$CACHE_ROOT/owl-chromium-runtime-macos-arm64-38a10eea4c9e.tar.gz"
  fi
  echo "Downloading patched Chromium Content Shell from $DOWNLOAD_URL"
  curl -fL "$DOWNLOAD_URL" -o "$archive"
  if [[ -n "$EXPECTED_ARCHIVE_SHA256" ]]; then
    local actual
    actual="$(shasum -a 256 "$archive" | awk '{print $1}')"
    if [[ "$actual" != "$EXPECTED_ARCHIVE_SHA256" ]]; then
      echo "error: Content Shell checksum mismatch. expected=$EXPECTED_ARCHIVE_SHA256 actual=$actual" >&2
      exit 1
    fi
  fi
...
verify_runtime_launch_policy() {
  ...
  for name in libowl_fresh_mojo_runtime.dylib owl-runtime-manifest.json owl-build-args.gn; do
    if [[ ! -f "$runtime_root/$name" ]]; then
      echo "error: Chromium OWL runtime is missing $name in $runtime_root" >&2
      exit 1
    fi
  done
  if ! python3 -m json.tool "$manifest" >/dev/null; then
    echo "error: Chromium OWL runtime manifest is not valid JSON: $manifest" >&2
    exit 1
  fi
  if strings "$dylib" | awk 'index($0, "no-sandbox") > 0 || index($0, "--no-sandbox") > 0 { found = 1 } END { exit found ? 0 : 1 }'; then
    echo "error: Chromium OWL runtime disables the Chromium sandbox with --no-sandbox" >&2
    exit 1
  fi
  if awk '$0 ~ /(^|[^A-Za-z0-9_-])(no-sandbox|--no-sandbox)([^A-Za-z0-9_-]|$)/ { found = 1 } END { exit found ? 0 : 1 }' "$build_args"; then
    echo "error: Chromium OWL build args contain no-sandbox" >&2
    exit 1
  fi
  if awk '$0 ~ /(^|[^A-Za-z0-9_-])(in-process-gpu|--in-process-gpu)([^A-Za-z0-9_-]|$)/ { found = 1 } END { exit found ? 0 : 1 }' "$build_args"; then
    echo "error: Chromium OWL build args enable in-process GPU by default" >&2
    exit 1
  fi
}
...
verify_rendering_path() {
  ...
  if ! strings "$framework_binary" | awk '$0 ~ /CALayerHost/ { layer = 1 } $0 ~ /IOSurface/ { surface = 1 } $0 ~ /Metal/ { metal = 1 } END { exit (layer && surface && metal) ? 0 : 1 }'; then
    echo "error: Chromium runtime does not expose the expected CALayerHost/IOSurface/Metal rendering path" >&2
    exit 1
  fi
}
...
verify_no_cef() {
  ...
  if find "$app_path" -path '*Chromium Embedded Framework.framework*' -print -quit | grep -q .; then
    echo "error: Chromium runtime contains Chromium Embedded Framework.framework" >&2
    exit 1
  fi
  local -a binaries=(
  ...
  for binary in "${binaries[@]}"; do
    if [[ -f "$binary" ]] &&
      strings "$binary" | awk '
        ...
        END { exit found ? 0 : 1 }
      '; then
      echo "error: Chromium runtime contains CEF markers in $binary" >&2
      exit 1
    fi
  done
}
